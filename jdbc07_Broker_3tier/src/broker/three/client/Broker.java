...
//	Database 		db;
	...
	/*
	 * 버튼을 Group(add, delete, update vs apply, cancel)하여 Enable되게 하는 메소드
	 */
	...
	/**
	 * //delete button에 의해 호출된다. 1)database의 deleteCustomer(ssn)call<BR>
	...
	*buyTf, sockTf, priceTf TextField에 뿌려준다.
	...
	@Override
	public void run() {
		// 무한루핑 돌면서 10초 간격으로 디비의 모든 주식을 긁어와서 뿌릴수 있도록 로직 추가
		while (true) {
			try {
				showStockList(db.getAllStocks(),stockList);
